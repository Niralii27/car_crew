...
class AdminEditSOSProduct extends StatefulWidget {
  final String productId;

  const AdminEditSOSProduct({Key? key, required this.productId})
      : super(key: key);

  @override
  State<AdminEditSOSProduct> createState() => _AdminEditSOSProductState();
}

class _AdminEditSOSProductState extends State<AdminEditSOSProduct> {
  ...
  final TextEditingController _includedDescriptionController =
      TextEditingController();
      ...
      if (productDoc.exists) {
        setState(() {
          ...
          _originalPriceController.text =
              (_productData!['originalPrice'] ?? 0.0).toString();
          _salesPriceController.text =
              (_productData!['salesPrice'] ?? 0.0).toString();
          ...
          if (_productData!['iconDescriptions'] != null) {
            final iconDescs =
                _productData!['iconDescriptions'] as Map<String, dynamic>;
          ...
          if (_productData!['includedDescriptions'] != null) {
            final includedDescs =
                _productData!['includedDescriptions'] as List<dynamic>;
            _includedDescriptions.clear();
            _includedDescriptions
                .addAll(includedDescs.map((desc) => desc.toString()));
          }
  ...
  Future<void> _updateProduct() async {
    ...
    try {
        ...
        SnackBar(
          content: Row(
            children: const [
              ...
              SizedBox(width: 10),
              Text('SOS Product updated successfully!'),
            ],
          ),
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Failed to update SOS product: $e')),
      );
    } finally {
