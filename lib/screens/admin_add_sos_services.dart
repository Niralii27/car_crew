...
class AdminAddSOSProduct extends StatefulWidget {
  const AdminAddSOSProduct({Key? key}) : super(key: key);

  @override
  State<AdminAddSOSProduct> createState() => _AdminAddSOSProductState();
}

class _AdminAddSOSProductState extends State<AdminAddSOSProduct> {
  ...
  Future<void> _saveProduct() async {
    ...
    try {
        ...
        SnackBar(
          content: Row(
            children: const [
              ...
              SizedBox(width: 10),
              Text('SOS Product added successfully!'),
            ],
          ),
      ...
      setState(() {
        ...
        _iconDescriptions.updateAll((key, value) => '');
        _includedDescriptions.clear();
      });
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Failed to save SOS product: $e')),
      );
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Add Car SOS Service Product'),
        elevation: 2,
      ),
      body: _isLoading
